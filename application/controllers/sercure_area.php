...
class Secure_area extends MX_Controller
{
	...
	*/
	function __construct($module_id=null,$submodule_id=null)
	{
		...
		$userid=$this->User->get_logged_in_User_info()->id;
		if(!$this->User->has_module_grant($userid,$module_id) && $module_id != 'home')
		{
		...
		$data['allowed_modules']=$this->Module->get_allowed_modules($logged_in_User_info->id);
		$data['backup_allowed']=false;
		foreach($data['allowed_modules']->result_array() as $module)
		{
			$data['backup_allowed']|=$module['id']==='config';
		}
